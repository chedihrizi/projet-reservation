...
$salle = $salleCtrl->getById($id);

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $result = $salleCtrl->update(
        $id, $_POST['nom'], $_POST['id_batiment'], $_POST['etage'], $_POST['capacite'],
...
<body>
    ...
    <main class="main-content">
        <div class="top-nav">
            <h2 style="font-size:18px;color:#381d51;">Modifier la Salle</h2>
        ...
        <div class="form-container">
            ...
            <form method="POST" onsubmit="return validateForm(event)">
                <div class="form-group">
                    ...
                    <input type="text" name="nom" id="nom" class="form-control" value="<?php echo htmlspecialchars($salle['nom']); ?>">
                </div>
                <div class="form-group">
                    <label>Bâtiment</label>
                    <select name="id_batiment" id="id_batiment" class="form-control">
                        <?php foreach ($batiments as $b): ?>
                        <option value="<?php echo $b['id']; ?>" <?php if ($b['id'] == $salle['id_batiment']) echo 'selected'; ?>><?php echo htmlspecialchars($b['nom']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label>Étage</label>
                        <input type="number" name="etage" id="etage" class="form-control" value="<?php echo $salle['etage']; ?>">
                    </div>
                    <div class="form-group">
                        <label>Capacité (personnes)</label>
                        <input type="number" name="capacite" id="capacite" class="form-control" value="<?php echo $salle['capacite']; ?>">
                    </div>
                </div>
                <div class="form-group">
                    <label>Équipements</label>
                    <input type="text" name="equipements" id="equipements" class="form-control" value="<?php echo htmlspecialchars($salle['equipements']); ?>">
                </div>
                <div class="form-group">
                    <label>Description</label>
                    <textarea name="description" id="description" class="form-control" rows="3"><?php echo htmlspecialchars($salle['description']); ?></textarea>
                </div>
                <div class="form-group">
                    <label>Options</label>
                    <div class="checkbox-group">
                        <input type="checkbox" name="disponible" id="disponible" <?php if ($salle['disponible']) echo 'checked'; ?>>
                        <label for="disponible">Disponible</label>
                    </div>
                    <div class="checkbox-group">
                        <input type="checkbox" name="statut_maintenance" id="statut_maintenance" <?php if ($salle['statut_maintenance']) echo 'checked'; ?>>
